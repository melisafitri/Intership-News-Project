...
function DetailNews() {
  ...
  const img = a.image_url || state?.img || drumpImage;
  const title = a.title || state?.title || "Memuat judul berita...";
  const source = a.news_origin?.origin || state?.source || "iNews";
  ...
  const topikMenarik = relatedData.slice(0, 5).map((item) => ({
    id: item.id,
    image: item.image,
    title: item.title,
  ...
  const relatedNewsSidebar = relatedData.slice(0, 4).map((item) => ({
    id: item.id,
    img: item.image,
    title: item.title,
        ...
        <article className="detail-news__container">
          ...
          <div className="detail-news__content">
            ...
            <div className="detail-news__meta">
              <Source text={`${category} | ${source}${date ? ` | ${date}` : ""}`} />
              <span className="detail-news__meta-separator"> | </span>
              <ReadingTime text={`Reading time ${minutes} menit`} />
            </div>
          ...
          {topikMenarik.length > 0 && (
            <div className="detail-news__related-list">
              <HorizontalNewsList title="Topik Menarik" news={topikMenarik} />
            </div>
          )}
        </article>

        <aside className="detail-news__sidebar">
          {relatedNewsSidebar.length > 0 && (
